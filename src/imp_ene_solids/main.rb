...
module Imp_EneSolidTools
  ...
  unless file_loaded?(__FILE__)
    ...
    if true # Sketchup.is_pro?
      ...
      item = menu.add_item("Multi-Subtract") { MultiSubTool.perform_or_activate }
      menu.set_validation_proc(item) { MultiSubTool.active? ? MF_CHECKED : MF_UNCHECKED }
      ...
      cmd.tooltip = "Intersect Improved"
      ...
      cmd = UI::Command.new("Multi") { MultiSubTool.perform_or_activate }
      cmd.large_icon = cmd.small_icon = File.join(PLUGIN_DIR, "images", "multisub.#{tb_extension}")
      cmd.tooltip = "Multi-Subtract"
      cmd.status_bar_text = "Subtract from multiple solid groups or components."
      cmd.set_validation_proc { MultiSubTool.active? ? MF_CHECKED : MF_UNCHECKED }
      tb.add_item cmd

    else
      UI.messagebox("Improved EneSolidTools in Extension Warehouse is only available for SketchUp Pro.")
    end
  end
